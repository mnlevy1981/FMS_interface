...
!> Use the correct file object
    ...
    IF ( time_ops ) THEN
       ...
       IF ( do_cf_compliance() ) THEN
          ! CF Compliance requires the unit on the _bounds axis is the same as 'time'
       ...
       ELSE
          files(file)%f_bounds =  write_field_meta_data(files(file)%file_unit,&
               & TRIM(time_name)//'_bounds', (/time_bounds_id,time_axis_id/),&
               & TRIM(time_unit_list(files(file)%time_units)),&
               & TRIM(time_name)//' interval endpoints', pack=pack_size, &
               & attributes=time_bounds_attributes, num_attributes=time_bounds_num_attributes, &
               & fileob=fileob)
       END IF
    END IF
...
!> Write data
    ...
    DO i = 1, files(file)%num_fields
       ...
       IF ( output_fields(num)%time_ops .AND. &
          ...
          ! time needs to be between start_dif and end_dif to prevent duplicate writes on time_bounds
          IF ( num == field ) THEN
